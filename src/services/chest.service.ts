...
const CHEST_COST_MONEY = 10000;

export const chestService = {
  async canOpenChest(userId: number): Promise<{ canOpen: boolean; reason?: string }> {
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { isVip: true, vipExpiresAt: true, lastChestOpenedAt: true, money: true },
    });

    if (!user) return { canOpen: false, reason: "Foydalanuvchi topilmadi!" };
    if (user.money < CHEST_COST_MONEY) return { canOpen: false, reason: `Yetarli pulingiz yo'q! (${CHEST_COST_MONEY}💰 kerak)` };

    const isVip = await vipService.isVip(userId);

    // VIP bo'lmasa — oyiga 1 marta
  ...
  async openChest(userId: number): Promise<{ success: boolean; reward?: ChestReward; error?: string }> {
    ...
    // Pul olish
    const spent = await economyService.spendMoney(userId, CHEST_COST_MONEY, "chest_open");
    if (!spent) return { success: false, error: "Yetarli pulingiz yo'q!" };
